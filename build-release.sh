...
LDFLAGS="-s -w -X github.com/psviderski/uncloud/internal/version.version=v${VERSION}"
...
go build -ldflags "${LDFLAGS}" -o "${DIST_DIR}/uc" ./cmd/uncloud
...
GOOS=linux GOARCH=amd64 go build -ldflags "${LDFLAGS}" -o "${DIST_DIR}/uc" ./cmd/uncloud
tar -czf "${DIST_DIR}/uc_linux_amd64.tar.gz" -C "${DIST_DIR}" uc
rm "${DIST_DIR}/uc"

GOOS=linux GOARCH=arm64 go build -ldflags "${LDFLAGS}" -o "${DIST_DIR}/uc" ./cmd/uncloud
...
GOOS=linux GOARCH=amd64 go build -ldflags "${LDFLAGS}" -o "${DIST_DIR}/uncloudd" ./cmd/uncloudd
tar -czf "${DIST_DIR}/uncloudd_linux_amd64.tar.gz" -C "${DIST_DIR}" uncloudd
rm "${DIST_DIR}/uncloudd"

GOOS=linux GOARCH=arm64 go build -ldflags "${LDFLAGS}" -o "${DIST_DIR}/uncloudd" ./cmd/uncloudd
